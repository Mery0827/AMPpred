...
# 当前文件所在目录: code/train/stage-2/V6/
...
EXPERIMENT_TAG = "v6_mtl_learnable_adaptive"  # 加了强先验锚定
...
# 结果保存到当前脚本同级目录（如 code/train/stage-2/V6/results/v6_mtl_learnable_adaptive/）
...
def get_task_alpha_config(pos_ratio):
    """
    返回先验 target_alpha 和对应的 logit 初始化。
    reg_weight 统一 0.5（足够强，防止坍缩）。
    """
    if pos_ratio < 0.15:
        target = 0.25  # Ab, Ef, Ml, Kp, St
        init_logit = np.random.uniform(-1.8, -1.0)
    elif pos_ratio < 0.40:
        target = 0.40  # Se, Bs
        init_logit = np.random.uniform(-0.6, 0.0)
    else:
        target = 0.65  # Pa, Sa, Ec
        init_logit = np.random.uniform(0.5, 1.2)

    return {
        'target_alpha': target,
        'init_logit': init_logit,
        'reg_weight': 0.5,  # 关键：提升到 0.5，与主损失同量级
        'alpha_lr': 2e-3
    }
...
def train_mtl_fold(fold_idx, all_data, fold_splits):
    print(f"\n{'#' * 70}")
    print(f"# FOLD {fold_idx + 1}/5 | Prior-Anchored Learnable α (V6-v6)")
    print(f"{'#' * 70}")
    ...
    for epoch in range(EPOCHS):
        ...
        print(
            f"  [Fold {fold_idx + 1}] Epoch {epoch + 1:3d} | Loss {avg_loss:.3f} | AvgMCC {avg_mcc:.3f} | AvgAUC {avg_auc:.3f} | AvgGMean {avg_gmean:.3f} | LR {lr_now:.6f} {alpha_str}")
    ...
    with torch.no_grad():
        for task_idx in range(NUM_BACTERIA):
            ...
            cv_records.append({
                ...
                'Val_AUC': m['AUC'],
                'Val_GMean': m['GMean'],
                'Val_Sen': m['Sensitivity'],
                'Val_Spe': m['Specificity'],
            })
    ...
    with torch.no_grad():
        for task_idx in range(NUM_BACTERIA):
            ...
            fold_results.append({
                ...
                'Val_MCC': cv_records[task_idx]['Val_MCC'],
                'Val_AUC': cv_records[task_idx]['Val_AUC'],
                'Val_GMean': cv_records[task_idx]['Val_GMean'],
            })

            print(f"     {BACTERIA_NAMES[task_idx]}: ValMCC={cv_records[task_idx]['Val_MCC']:.3f} "
                  f"TestMCC={test_m['MCC']:.3f} α={final_alpha:.3f} target={target_alpha:.2f}")
...
def main():
    ...
    print("=" * 80)
    print(" Deep-AMPpred Stage-2 | V6-v6: Prior-Anchored Learnable α")
    print("=" * 80)
    print(f"Version: {EXPERIMENT_TAG}")
    print(f"Loss: LEARNABLE_ADAPTIVE=True | AUX={USE_MIC_AUX}")
    print(f"Device: {device}")
    print(f"Feature Dir: {FEATURE_DIR}")
    print(f"Save Dir: {SAVE_DIR}")
    print(f"Key Fix: logit-space prior | reg=0.5 | Kendall-style uncertainty")
    print("=" * 80)

    all_data = load_all_bacteria_data()

    print("\n📋 Task Imbalance Preview & Alpha Config (V6-v6):")
